...
```mermaid
flowchart TD
  Select[Chọn nhóm kỹ thuật] --> Table[Bảng DMKT]
  Table --> Upload[Upload Excel]
  Table --> EditInline[Sửa trên bảng chính]
  Upload --> Validate{Validate file}
  Validate -->|Có lỗi| Tooltip[Tooltip lỗi theo dòng]
  Tooltip --> Upload
  Validate -->|Hợp lệ| Preview[Side panel preview]
  Preview --> Close{Đóng chưa lưu?}
  Close -->|Có| Confirm[Popup xác nhận]
  Preview --> Save[Lưu cập nhật]
...
```mermaid
flowchart LR
  NK[Quản lý Nhóm kỹ thuật<br/>dropdown nguồn]
